...
N <- hh %>% dplyr::summarize(Count = sum(nhhmem)) %>% 
  ...
  rbind.data.frame(hh %>% group_by(as_factor(age1)) %>% 
                     dplyr::summarize(Count = sum(nhhmem)) %>% select(Count)) %>% 
  
  rbind.data.frame(hh %>% group_by(as_factor(drinkingWater)) %>% 
                     dplyr::summarize(Count = sum(nhhmem)) %>% select(Count)) %>% 
  
  rbind.data.frame(hh %>% group_by(as_factor(HHSEX)) %>% 
                     dplyr::summarize(Count = sum(nhhmem)) %>% select(Count)) %>% 
...
N <- data.frame(N); colnames(N)[1] = label(hh$nhhmem)
rownames(N) = c("Total", levels(as_factor(hh$HH6)), levels(as_factor(hh$HH7)), levels(as_factor(hh$helevel)), levels(as_factor(hh$age1)),
                levels(as_factor(hh$drinkingWater)), levels(as_factor(hh$HHSEX)),
