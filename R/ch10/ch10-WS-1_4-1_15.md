...
<th style="border-bottom:hidden; padding-bottom:0; padding-left:3px;padding-right:3px;text-align: center; " colspan="13">

<div style="border-bottom: 1px solid #ddd; padding-bottom: 5px; ">

Table WS 2.1: Handwashing facility with soap and water on premises
<br><br> Percent distribution of household members by observation of
handwashing facility and percentage of household members by availability
of water and soap or detergent at the handwashing facility
...
No handwashing facility observed in the dwelling, yard, or plot

</th>

<th style="text-align:left;">

No permission to see/Other
...
with ash/mud/sand available

</th>

<th style="text-align:left;">

Number of household members where handwashing facility was observed

</th>

<th style="text-align:left;">

Percentage of household members with handwashing facility where water
and soap are present\[1\]

</th>

<th style="text-align:left;">

Number of household members where handwashing facility was observed or
with no handwashing facility in the dwelling, yard, or plot
...
val_lab(hh$WS11) = make_labels("    
    11 CHASSE D’EAU: FLUSH TO PIPED SEWER SYSTEM
    12 CHASSE D’EAU: FLUSH TO SEPTIC TANK
    13 CHASSE D’EAU: FLUSH TO PIT LATRINE
    14 CHASSE D’EAU: FLUSH TO OPEN DRAIN
    18 CHASSE D’EAU: FLUSH TO DK WHERE
    22 LATRINE A FOSSE: PIT LATRINE WITH SLAB
    23 LATRINE A FOSSE: PIT LATRINE WITHOUT SLAB / OPEN PIT
...
row.vars = list(total(), hh$HH6, hh$HH7, hh$helevel, hh$WS14, hh$windex5)
...
cell.vars = hh$nn
  ...
  taba = calc_cro_rpct(hh, hh$nn,
                 row_vars = row.vars, 
                 col_vars = list(hh$WS11,total()), 
                 weight = hh.weight, total_row_position = "none") %>% data.frame() %>% 
  select(2, 3, 4, 6, 7, 5, 8, 10, 11, 9, 12) %>% 
  round(digits = 1)

tabb = calc_cro_rpct(hh, hh$nn, row_vars = row.vars,
                      col_vars = hh$improvepercent, weight = hh.weight, total_row_position = "none") %>% 
  data.frame() %>% select(contains(c("Total", "100"))) %>% round(digits = 1)



tabc = calc_cro_cases(hh, cell_vars = hh$nn,
                      row_vars = row.vars,
                      col_vars = list(total()),
                      weight = hh.weight, 
                      total_row_position = "none") %>% data.frame() %>% select(contains(c("Total", "100"))) %>% round(digits = 0)




tab = cbind(taba, tabb, tabc)
...
colnames(tab) = c(levels(as_factor(hh$WS11))[c(2, 3, 4, 6, 7, 5, 8, 10, 11, 9) -1], "Total", label(hh$improvepercent), label(hh$nhhmem))

rownames(tab) = c("Total", levels(as_factor(hh$HH6)), levels(as_factor(hh$HH7)),
                   levels(as_factor(hh$helevel)), levels(as_factor(hh$WS14)),
                   levels(as_factor(hh$windex5)))
...
tab %>% 
  ...
  pack_rows(label(hh$HH7), 4, 10) %>% 
  pack_rows(label(hh$helevel), 11, 15) %>%
  pack_rows(label(hh$WS14), 16, 20) %>% 
  pack_rows(label(hh$windex5), 21, 25) %>% 
  add_header_above(c(" " = 1, "Improved Sanitation facility" = 5, "Unimproved Sanitation facility" = 4, " " = 4)) %>% 
  add_header_above(c("Type of sanitation facility used by household" = 14)) %>% 
  add_header_above(c("Table WS 3.1: Use of improved and unimproved sanitation facilities \n
                     Percent distribution of household population according to type of sanitation facility 
                     used by the household" = 14))
```
...
<th style="border-bottom:hidden; padding-bottom:0; padding-left:3px;padding-right:3px;text-align: center; " colspan="14">

<div style="border-bottom: 1px solid #ddd; padding-bottom: 5px; ">

Table WS 3.1: Use of improved and unimproved sanitation facilities
<br><br> Percent distribution of household population according to type
of sanitation facility <br> used by the household
...
CHASSE D’EAU: FLUSH TO PIPED SEWER SYSTEM

</th>

<th style="text-align:left;">

CHASSE D’EAU: FLUSH TO SEPTIC TANK

</th>

<th style="text-align:left;">

CHASSE D’EAU: FLUSH TO PIT LATRINE

</th>

<th style="text-align:left;">

CHASSE D’EAU: FLUSH TO DK WHERE

</th>

<th style="text-align:left;">

LATRINE A FOSSE: PIT LATRINE WITH SLAB

</th>

<th style="text-align:left;">

CHASSE D’EAU: FLUSH TO OPEN DRAIN

</th>

<th style="text-align:left;">

LATRINE A FOSSE: PIT LATRINE WITHOUT SLAB / OPEN PIT
...
Percentage using improved sanitation \[1\]
...
<strong>Location of sanitation facility</strong>
...
Have no idea where this discrepancy is coming from in 3.2, cannot track
down in source data or even get close to percentages using source survey
variables - revisit later
...
hh = hh %>% mutate(shared1 = ifelse(toiletType == 1, sharedToilet, 0))
...
hh = hh %>% mutate(shared2 = ifelse(toiletType == 2, sharedToilet, 0))
...
hh = hh %>% mutate(shared3 = ifelse(toiletType == 3, 1, 0))
var_lab(hh$shared3) = ""
val_lab(hh$shared3) = make_labels("1 Open defecation (no facility, bush, field)")
...
cell.vars = hh$nn
  ...
  taba = calc_cro_rpct(hh, hh$nn,
                 row_vars = row.vars, 
                 col_vars = list(hh$shared1, hh$shared2, hh$shared3, total()), 
                 weight = hh.weight, total_row_position = "none") %>% data.frame() %>% 
  select(2:5, 7, 8, 13, 14) %>% 
  round(digits = 1)
...
colnames(tab) = c(levels(as_factor(hh$shared1))[1:4], levels(as_factor(hh$shared2))[1:2], levels(as_factor(hh$shared3))[2], "Total", label(hh$nhhmem))
...
tab %>% 
  ...
  pack_rows(label(hh$WS14), 16, 20) %>% 
  pack_rows(label(hh$windex5), 21, 25) %>% 
  add_header_above(c(" " = 1, "Users of improved Sanitation facilities" = 5, "Users of unimproved sanitation facilities" = 2, " " = 2)) %>% 
  add_header_above(c("Table WS 3.2: Use basic and limited sanitation services \n
                     Percent distribution of household population by use of private and public sanitation facilities and use of shared 
                     facilities, by users of improved and unimproved sanitation facilities" = 10))
```
...
Table WS 3.2: Use basic and limited sanitation services <br><br> Percent
distribution of household population by use of private and public
sanitation facilities and use of shared <br> facilities, by users of
...
Shared by: More than 5 households
...
var_lab(hh$septicemptiedproviderplant) = "Removed by a service provider to treatment"
var_lab(hh$septicemptiedproviderDK) = "Removed by a service provider to DK"
var_lab(hh$septicemptiedpit) = "Buried in a covered pit"
var_lab(hh$septicemptiedHHpit) = "To uncovered pit, open ground, water body or elsewhere"
var_lab(hh$septicemptiedother) = "Other"

var_lab(hh$septicemptiedDK) = "DK where wastes were taken"

var_lab(hh$septicneveremptied) = "Never emptied"
var_lab(hh$septicDKemptied) = "DK if ever emptied/Missing"

var_lab(hh$Oonsiteemptiedproviderplant) = "Removed by a service provider to treatment"
var_lab(hh$OonsiteemptiedproviderDK) =  "Removed by a service provider to DK"
var_lab(hh$Oonsiteemptiedpit) = "Buried in a covered pit"
var_lab(hh$OonsiteemptiedHHpit) = "To uncovered pit, open ground, water body or elsewhere"

var_lab(hh$Oonsiteemptiedother) = "Other"
var_lab(hh$OonsiteemptiedDK) = "DK where wastes were taken"

var_lab(hh$Oonsiteneveremptied) = "Never emptied"
var_lab(hh$OonsiteDKemptied) = "DK if ever emptied/Missing"
var_lab(hh$safedisposal) = "Safe disposal in situ of excreta from on-site sanitation facilities [1]"
var_lab(hh$unsafedisposal) = "Unsafe disposal of excreta from on-site sanitation facilities"
var_lab(hh$treatment) = "Removal of excreta for treatment from on-site sanitation facilities"

val_lab(hh$layer1) = make_labels("
                                 0 Emptying and disposal of wastes from septic tanks
                                 ") 


val_lab(hh$layer2) = make_labels("
                                 0 Emptying and disposal of wastes from other improved on-site sanitation facilities
                                 ") 




val_lab(hh$onsite) = make_labels("
...
``` r
ws11recode = mrset(hh$onsite, hh$WS11c)

hh$nn = 1


row.vars = list(total(), hh$HH6, hh$HH7, hh$helevel, ws11recode, hh$windex5)
...
cell.vars = hh$nn
  ...
  taba = calc_cro_rpct(hh, hh$nn,
                 row_vars = row.vars, 
                 col_vars = list(hh$septicemptiedproviderplant, hh$septicemptiedproviderDK, hh$septicemptiedpit, hh$septicemptiedHHpit,
                                 hh$septicemptiedother, hh$septicemptiedDK, hh$septicneveremptied, hh$septicDKemptied), 
                 weight = hh.weight, total_row_position = "none") %>% data.frame() %>% slice(-c(18,20,22)) %>% 
  select(contains(c("Total", "100"))) %>% 
  round(digits = 1)

  
tabb = calc_cro_rpct(hh, hh$nn,
                 row_vars = row.vars, 
                 col_vars = list(hh$Oonsiteemptiedproviderplant, hh$OonsiteemptiedproviderDK, hh$Oonsiteemptiedpit, hh$OonsiteemptiedHHpit,
                                 hh$Oonsiteemptiedother, hh$OonsiteemptiedDK, hh$Oonsiteneveremptied, hh$OonsiteDKemptied, total()), 
                 weight = hh.weight, total_row_position = "none") %>% data.frame() %>% slice(-c(18,20,22)) %>% 
  select(contains(c("100", "Total"))) %>% 
  round(digits = 1)



tabc = calc_cro_rpct(hh, hh$nn,
                     row_vars = row.vars,
                     col_vars = list(hh$safedisposal, hh$unsafedisposal, hh$treatment),
                     weight = hh.weight, total_row_position = "none") %>% data.frame() %>% slice(-c(18,20,22)) %>% 
  select(contains(c("Total", "100"))) %>% 
  round(digits = 1)


tabd = calc_cro_cases(hh, hh$nn, row_vars = row.vars, col_vars = total(), 
                      weight = hh.weight, total_row_position = "none") %>%
  data.frame() %>% slice(-c(18,20,22)) %>% select(contains(c("Total", "100"))) %>% 
  round(digits = 0)
  


tab = cbind(taba, tabb, tabc, tabd)


colnames(tab) = gsub(".", " ", colnames(tab), fixed = TRUE)
colnames(tab) = gsub("100", " ", colnames(tab), fixed = TRUE)
colnames(tab) = gsub("1", " ", colnames(tab), fixed = TRUE)
colnames(tab)[16] = "Total"; colnames(tab)[20] = "Number of household members in households with improved on-site sanitation facilities"
...
#colnames(tab) = sapply(1:length(col.vars), function(i) label(col.vars[[i]]))

#colnames(tab) = c(label(hh$septicemptiedproviderplant), label(hh$septicemptiedproviderDK), label(hh$septicemptiedpit),
#                  label(hh$septicemptiedHHpit), label(hh$septicemptiedother), label( hh$septicemptiedDK),
#                  label(hh$septicneveremptied), label(hh$septicDKemptied), label(hh$Oonsiteemptiedproviderplant), )

rownames(tab) = c("Total", levels(as_factor(hh$HH6)), levels(as_factor(hh$HH7)),
                   levels(as_factor(hh$helevel)), levels(as_factor(hh$onsite))[c(1:3, 5)],
                   levels(as_factor(hh$windex5)))
...
tab %>% 
  ...
  pack_rows(label(hh$HH7), 4, 10) %>% 
  pack_rows(label(hh$helevel), 11, 15) %>%
  pack_rows(label(hh$onsite), 16, 19) %>% 
  pack_rows(label(hh$windex5), 20, 24) %>% 
  add_header_above(c(" " = 1, "Emptying and disposal of wastes from septic tanks" = 8,
                     "Emptying and disposal of wastes from other improved on-site sanitation facilities" = 8, " " = 4)) %>% 
  add_header_above(c("Table WS 3.3: Emptying and removal of excreta from on-site sanitation facilities \n
                     Percent distribution of household members in households with septic tanks and improved latrines by method of emptying and removal" = 21))
```
...
<th style="border-bottom:hidden; padding-bottom:0; padding-left:3px;padding-right:3px;text-align: center; " colspan="21">

<div style="border-bottom: 1px solid #ddd; padding-bottom: 5px; ">

Table WS 3.3: Emptying and removal of excreta from on-site sanitation
facilities <br><br> Percent distribution of household members in
households with septic tanks and improved latrines by method of emptying
and removal
...
Emptying and disposal of wastes from other improved on-site sanitation
facilities
...
Removed by a service provider to treatment

</th>

<th style="text-align:left;">

Removed by a service provider to DK
...
To uncovered pit open ground water body or elsewhere

</th>

<th style="text-align:left;">

DK where wastes were taken
...
Unsafe disposal of excreta from on site sanitation facilities

</th>

<th style="text-align:left;">

Removal of excreta for treatment from on site sanitation facilities

</th>

<th style="text-align:left;">

Number of household members in households with improved on-site
...
<strong>Type of onsite sanitation facility</strong>
...
var_lab(hh$safedisposal) = "Safe disposal in situ of excreta from on-site sanitation facilities"
var_lab(hh$unsafedisposal) = "Unsafe disposal of excreta from on-site sanitation facilities"
var_lab(hh$treatment) = "Removal of excreta for treatment off-site [1]"
var_lab(hh$connectedsewer) = "Connected to sewer"
var_lab(hh$usingimproved) = "Using unimproved sanitation facilities"

var_lab(hh$opendefecation) = "Practising open defecation"
var_lab(hh$missingx) = "Missing"
```
...
col.vars = list(hh$safedisposal, hh$unsafedisposal, hh$treatment, hh$connectedsewer,
                  hh$usingimproved, hh$opendefecation, hh$missingx)
...
cell.vars = hh$nn
  ...
  taba = calc_cro_rpct(hh, hh$nn,
                 row_vars = row.vars, 
                 col_vars = list(
                  hh$safedisposal, hh$unsafedisposal, hh$treatment, hh$connectedsewer,
                  hh$usingimproved, hh$opendefecation, hh$missingx, total()
                   ), 
                 weight = hh.weight, total_row_position = "none") %>% 
    data.frame() %>% select(contains(c("100", "Total"))) %>% 
  round(digits = 1)

  
tabb = calc_cro_cases(hh, hh$nn,
                 row_vars = row.vars, 
                 col_vars = list(
                   total()
                   ), 
...
colnames(tab) = c(sapply(1:length(col.vars), function(i) label(col.vars[[i]])), "Total", label(hh$nhhmem))
...
tab %>% 
  ...
  pack_rows(label(hh$helevel), 11, 15) %>%
  pack_rows(label(hh$windex5), 16, 20) %>% 
  add_header_above(c(" " = 1, "Using improved on-site sanitation systems" = 3, " " = 6)) %>%  
  add_header_above(c("Table WS 3.4: Management of excreta from household sanitation facilities \n
                     Percent distribution of household population by management of excreta from household sanitation facilities" = 10))
```
...
<br><br> Percent distribution of household population by management of
excreta from household sanitation facilities
...
<th style="border-bottom:hidden" colspan="6">
...
Removal of excreta for treatment off-site \[1\]
...
Using unimproved sanitation facilities
...
var_lab(hh$stools) =  "Percentage of children whose last stools were disposed of safely [A]"
var_lab(hh$disposalplace) =  "Place of disposal of child's faeces"
val_lab(hh$disposalplace) = make_labels("
...
hh$numChildren = 1
var_lab(hh$numChildren) = "Number of children age 0-2 years"
```
...
row.vars = list(total(), hh$HH6.x, hh$HH7.x, hh$melevel, hh$toiletType, hh$windex5.x)

# col.vars = list(hh$disposalplace, "total", hh$stools, hh$nn)

hh.weight = hh$chweight
cell.vars = hh$numChildren
...
taba = calc_cro_rpct(
  ...
  row_vars = row.vars, 
  col_vars = list(
    hh$disposalplace, total(), hh$stools
    
  ),
  weight = hh.weight,
  total_row_position = "none"
) %>% data.frame() %>% select(-c(1,8)) %>% round(digits = 1)
...
tabb = calc_cro_cases(
  ...
  weight = hh.weight,
  total_row_position = "none"
) %>% data.frame() %>% select(contains(c("100", "Total"))) %>% round(digits = 0)
...
rownames(tab) = c("Total", levels(as_factor(hh$HH6.x)), levels(as_factor(hh$HH7.x)),
                   levels(as_factor(hh$melevel)), levels(as_factor(hh$toiletType)),
                   levels(as_factor(hh$windex5.x)))



colnames(tab) = c(levels(as_factor(hh$disposalplace)), "Total", label(hh$stools), label(hh$numChildren))
...
tab %>% 
  kable(digits = 1, format.args = list(big.mark = ",", scientific = FALSE)) %>% 
  kable_styling(c("striped"), font_size = 12) %>% 
  pack_rows(label(hh$HH6.x), 2, 3) %>% 
  pack_rows(label(hh$HH7.x), 4, 10) %>% 
  pack_rows(label(hh$melevel), 11, 15) %>%
  pack_rows(label(hh$toiletType), 16, 18) %>% 
  pack_rows(label(hh$windex5.x), 19, 23) %>% 
  add_header_above(c(" " = 1, "Place of disposal of child's faeces" = 8, " " = 3)) %>%  
  add_header_above(c("Table WS 3.5: Disposal of child's faeces \n
                     Percent distribution of children age 0-2 years according to place of disposal of child's faeces, and the percentage of children age 
                     0-2 years whose stools were disposed of safely the last time the child passed stools" = 12))
```
...
Table WS 3.5: Disposal of child’s faeces <br><br> Percent distribution
of children age 0-2 years according to place of disposal of child’s
faeces, and the percentage of children age <br> 0-2 years whose stools
were disposed of safely the last time the child passed stools
...
Percentage of children whose last stools were disposed of safely \[A\]
...
<strong>Type of sanitation facility</strong>
